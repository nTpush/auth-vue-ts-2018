<template>
    <div class="layout">
        <Button type="primary" @click="show" size="small"
                :disabled="(storeNode.indexOf(25) === -1) || (Number(row.type) !== 1)">密码重置</Button>
        <Button type="success" @click="handleSetRole" size="small" v-if="!row.roles.length"
                :disabled="(storeNode.indexOf(26) === -1) || (Number(row.type) !== 1)">设置角色</Button>
